...
# from dataset_multi import PairedDataset # for multi-view
...
#################################################################################################################
    ...
    # sampler = torch.utils.data.RandomSampler(dataset_train, replacement=True, num_samples=100000)
...
#################################################################################################################
    ...
    if accelerator.is_main_process:
        ...
        os.makedirs(os.path.join(args.output_dir, f"checkpoints_{time}"), exist_ok=True)
        ############################################################################################
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            if global_step >= args.max_train_steps: # to mitigate dataloader change
                outf = os.path.join(args.output_dir, f"checkpoints_{time}", "model.pkl")
            ...
            with accelerator.accumulate(*l_acc):
                ...
                if args.mask:
                    ...
                    mask_ratio = ratio  # 30% 마스크
                    mask = (torch.rand((B, 1, H, W), device=x_src.device) < mask_ratio)  # True/False mask
                    mask = mask.unsqueeze(2).repeat(1, 1, C, 1, 1)  # (B, 1, C, H, W), batch-wise masking to the first image

                    x_src[:, 0] = x_src[:, 0].masked_fill(mask[:, 0], 0.0)
                ...
                if args.lambda_gram > 0:
                    if global_step > args.gram_loss_warmup_steps:
                        ...
                        crop_h, crop_w = min(400, H), min(400, W) # adjust to the input image size
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 1: # 0 -> 0
                        ...
                        try:
                            ...
                            print(f"Error saving checkpoint at step {global_step}: {e}")
...
if __name__ == "__main__":
    ...
    # additional options
    parser.add_argument("--mask", action="store_true", help="Whether or not to use mask to the input image.")
