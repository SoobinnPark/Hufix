...
if __name__ == "__main__":
    ...
    if os.path.isdir(args.input_image):
        ...
        # print(f"Loading input images is successful, #:{len(input_images)}")
        
        print("Searching in:", os.path.join(args.input_image, "*.png"))
        input_images = sorted(glob(os.path.join(args.input_image, "*.png")))
        print("Found images:", input_images)
    ...
    if args.ref_image is not None:
        if os.path.isdir(args.ref_image):
            ...
            ref_images = sorted(glob(os.path.join(args.ref_image, "*frame0000*_gt.png")))
            ref_images = ref_images[:4]
            print(f"Found {len(ref_images)} reference images (frame0000 gt):")
        else:
